...
JVM内存最好不要超过32G
在 32位操作系统中，指针是4个字节的，由于cpu中最小的寻址单位为 1比特 byte, 所以能够使用的最大内存为 (4字节即4*8=32位)2^32*1byte = 4GB
在 64位操作系统中，开启指针压缩后，其指针同样式 4个字节，但是由于其 java 对象进行了 8字节映射地址，所以可用的地址就是 2^32*1byte = 32GB
所以指针压缩后，最大的可用内存为32GB,超过32GB 后指针压缩就失效了。

对象的静态变量，即类属性，可以压缩的对象
对象头   64位的情况下，原生对象头 16压缩为 12字节
引用类型 引用类型即指针， 8字节压缩为 4字节
数组    数组类型大小由 24字节压缩为 16字节
不可压缩的对象
指向非 heap 的对象指针
局部变量 传参 返回值 null 指针
